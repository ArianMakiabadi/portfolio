...
- Custom `.cur` cursors: the default arrow is set globally on `html, body, #root`; add the `pointer` class (not `cursor-pointer`) to anything clickable, and `progress` for waiting states — for an app-wide wait cursor during a window's loading phase, use the `useProgressCursor` hook (see Loading states) rather than toggling the class by hand.
